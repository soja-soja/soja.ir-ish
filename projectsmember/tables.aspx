                        ...
                        <div class="card my-4">
                            ...
                            <div class="card-body px-0 pb-2">
                                <div class="table-responsive">
                                    ...
                                    <asp:SqlDataSource runat="server" ID="SqlDataSourceFullProjects" ConnectionString='<%$ ConnectionStrings:db_a12df8_sojairishConnectionString4 %>' DeleteCommand="DELETE FROM [Projects] WHERE [Id] = @Id" InsertCommand="INSERT INTO [Projects] ([ReferralSource], [ProjectType], [Budget], [Timeline], [ProjectFeatures], [Email], [Phone], [checked], [contacted], [engaged], [offeredToMembers], [Owners], [Notes]) VALUES (@ReferralSource, @ProjectType, @Budget, @Timeline, @ProjectFeatures, @Email, @Phone, @checked, @contacted, @engaged, @offeredToMembers, @Owners, @Notes)" SelectCommand="SELECT * FROM [Projects]" UpdateCommand="UPDATE [Projects] SET [ReferralSource] = @ReferralSource, [ProjectType] = @ProjectType, [Budget] = @Budget, [Timeline] = @Timeline, [ProjectFeatures] = @ProjectFeatures, [Email] = @Email, [Phone] = @Phone, [checked] = @checked, [contacted] = @contacted, [engaged] = @engaged, [offeredToMembers] = @offeredToMembers, [Owners] = @Owners, [Notes] = @Notes WHERE [Id] = @Id">
                                        <DeleteParameters>
                                            <asp:Parameter Name="Id" Type="Int32"></asp:Parameter>
                                        </DeleteParameters>
                                        <InsertParameters>
